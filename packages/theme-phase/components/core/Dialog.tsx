...
export function DialogContent({ className = '', children }: DialogContentProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      {/* Overlay */}
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm animate-fadeIn" onClick={() => setOpen(false)} aria-hidden="true" />
...
export function DialogBody({ className = '', children }: DialogBodyProps) {
  return <div className={`px-6 py-4 flex-1 overflow-y-auto min-h-0 ${className}`.trim()}>{children}</div>;
}
